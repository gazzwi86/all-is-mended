...
const paths = {
  client: {
    ...
    styles: [`${clientPath}/app/**/*.scss`, '!${clientPath}/app/styles/'],
    mainStyle: `${clientPath}/app/app.scss`,
    views: `${clientPath}/app/**/*.jade`,
      ...
      .pipe(plugins.inject(
          gulp.src(`/${clientPath}/app/**/*.css`, { read: false })
            .pipe(plugins.sort()), {
              ...
              endtag: '<!-- endinjector -->',
              transform: (filepath) => '<link rel="stylesheet" href="' + filepath.replace(`/${clientPath}/`, '')
                .replace('/.tmp/', '') + '">'
            }))
        .pipe(gulp.dest(clientPath));
});
...
gulp.task('lint:scripts', ['jscs', 'eslint'], cb => runSequence(['lint:scripts:client', 'lint:scripts:server'], cb));

gulp.task('lint:scripts:client', () => {
  return gulp.src(_.union(
          paths.client.scripts,
          _.map(paths.client.test, blob => '!' + blob), [`!${clientPath}/app/constants/app.constant.js`]
      ))
...
gulp.task('build', cb => {
  console.warn('Have you generated favicons yet?');
  runSequence(
      'test',
      'clean:dist',
      'clean:tmp',
...
gulp.task('html', function() {
  return gulp.src([`${clientPath}/**/*.html`, '!' + paths.client.mainView])
        .pipe(plugins.angularTemplatecache({
          module: 'allIsMendedApp'
        }))
        .pipe(gulp.dest('.tmp'));
});
...
gulp.task('constant', function() {
  ...
  return plugins.ngConstant({
    ...
    .pipe(gulp.dest(`${clientPath}/app/constants/`));
});
...
// favicon
gulp.task('favicon', () => {
  return gulp.src(`${clientPath}/assets/images/logo.png`)
        .pipe(plugins.favicons({
          appName: 'All Is Mended',
          appDescription: '',
          developerName: 'Itinerate Consulting',
          developerURL: 'http://www.itinerateconsulting.com/',
          background: '#fff',
          path: '/assets/images/favicons/',
          url: 'http://allismended.com/',
          display: 'standalone',
          orientation: 'portrait',
          version: 1.0,
          logging: false,
          online: false,
          html: paths.client.mainView,
          pipeHTML: false,
          replace: false
        }))
        .on('error', err => {
        ...
        .pipe(gulp.dest('client/assets/images/favicons/'));
});
